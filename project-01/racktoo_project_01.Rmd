...
To get a better idea of the structure of the data within the dataframe, we cause use the *str()* function.
...
ggplot(data = avg_mpg_over_time, aes(x = year, y = avg_mpg, group = 1)) +
...
```{r}
ggplot(data = avg_disp_over_time, aes(x = year, y = avg_disp, group = 1)) +
...
The plots tell a story about the development of gasoline vehicles in response to world events and public sentiments. In the *Average MPG per year* plot, the first question posited is answered, as fuel efficiency has indeed seen massive improvements in the last decade. We can overlay the average displacement plot upon the MPG plot to visualize a relationship between the two:
...
In this plot, we see that engine displacement was typically inversely proportional to fuel efficiency. During the '80s and '90s, for instance, a growing desire for faster, more powerful vehicles saw a significant decline in MPG ratings while larger, higher-displacement engines were produced. This trend continued until the middle of the '90s, when interest started to grow in efficient, low-cost vehicles. 
